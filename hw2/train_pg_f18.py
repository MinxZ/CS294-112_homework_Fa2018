...
def normalize(values, mean=0.0, std=1.0):
    ...
    _std = np.std(values)
    std_away = (values - _mean) / (_std + 1e-8)
...
class Agent(object):
    ...
    def sample_action(self, policy_parameters):
        ...
        if self.discrete:
            ...
            # YOUR_CODE_HERE
            # sy_sampled_ac = tf.reduce_max(sy_logits_na, axis=-1)
    ...
    def get_log_prob(self, policy_parameters, sy_ac_na):
        ...
        if self.discrete:
            ...
            # sy_logprob_n = tf.log(self.sample_action(sy_logits_na))
        else:
            sy_mean, sy_logstd = policy_parameters
            # YOUR_CODE_HERE
            sy_x = (sy_ac_na - sy_mean) / tf.exp(sy_logstd)
            sy_logprob_n = -0.5 * tf.reduce_sum(tf.square(sy_x), axis=1)
        return sy_logprob_n
    ...
    def build_computation_graph(self):
        ...
        if self.nn_baseline:
            ...
            baseline_loss = tf.nn.l2_loss(self.baseline_prediction - self.sy_target_n)  # YOUR CODE HERE
